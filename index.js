...
async function summarize_messages(indexes = null, show_progress = true, api_keys = []) {
    ...
    // If it's not provided/empty (e.g. from callers like slash commands),
    // this function will internally call get_api_keys() to fetch them from UI settings.
    let ctx = getContext();

    let actual_raw_keys_to_use;
    // Check if api_keys parameter was provided, is an array, and is non-empty
    if (api_keys && Array.isArray(api_keys) && api_keys.length > 0) {
        ...
        // Fallback to fetching from UI if api_keys param is not valid or not provided
    ...
    // This is the crucial boolean that determines the execution path.
    // It replaces any older variables like `using_multiple_api_keys`.
    ...
    // const use_direct_google_api definition is now at the top of the function, based on actual_raw_keys_to_use
    ...
    // This logic is for the ST FALLBACK PATH, correctly gated by `!use_direct_google_api`
    if (!use_direct_google_api) {
        ...
        if (!await verify_connection_profile(settings_connection_profile)) {
            ...
            // No need to restore here, as the finally block will handle it.
            ...
            // Return or throw is not strictly needed here due to the finally block ensuring state restoration.
            // However, to prevent further execution in this try block, we can return.
            return;
        }
    ...
    } else { // Fallback to old logic (ST profile-based summarization)
        debug("Using fallback ST profile-based summarization.");
        // Fallback logic (single ST profile from settings) is already covered by the `if (!use_direct_google_api)`
        // block at the beginning of the `try`. If that block executed, ST profile and preset are set.
        // If it didn't execute (because use_direct_google_api was true), this `else` block is skipped.
    ...
    if (STOP_SUMMARIZATION) {
        STOP_SUMMARIZATION = false; // Reset flag
